...
def main():
    ...
    # create model
    model_quan = models.__dict__[args.arch]()
    model_pretrained = models.__dict__[args.arch]()
    assert(args.pretrained is not '')
    model_quan.load_state_dict(torch.load(args.pretrained))
    ...
    #### Weight Quantization
    ############################
    conv = model_pretrained.conv1
    conv_quan = model_quan.conv1
    conduct_ofwa(train_loader, model_pretrained, model_quan, conv, conv_quan, None, 8, prefix=args.prefix+'/conv1', ec=False)

    #### quantize 4 blocks
    for layer_idx in range(1, 5):
        current_layer_pretrained = eval('model_pretrained.layer{}'.format(layer_idx))
        current_layer_quan = eval('model_quan.layer{}'.format(layer_idx))
        for block_idx in range(len(current_layer_pretrained)):
            ...
            conduct_ofwa(train_loader, model_pretrained, model_quan, conv, conv_quan, q_module, 
                         args.weight_bit_width, prefix=args.prefix+'/layer'+str(layer_idx)+'_block'+str(block_idx)+'_conv1', ec=False)
            ...
            conduct_ofwa(train_loader, model_pretrained, model_quan, conv, conv_quan, q_module, 
                         args.weight_bit_width, prefix=args.prefix+'/layer'+str(layer_idx)+'_block'+str(block_idx)+'_conv2', ec=False)
            ...
            if current_block_pretrained.downsample is not None:
                ...
                conduct_ofwa(train_loader, model_pretrained, model_quan, conv, conv_quan, None, 
                             args.weight_bit_width, prefix=args.prefix+'/layer'+str(layer_idx)+'_block'+str(block_idx)+'_downsample', ec=False)

    ## quantize last fc
    conv = model_pretrained.fc
    conv_quan = model_quan.fc
    q_module = model_quan.quan
    conduct_ofwa(train_loader, model_pretrained, model_quan, conv, conv_quan, q_module, 8, prefix=args.prefix+'/fc', ec=False)

    ## Load Weight Quantization
    conv = model_pretrained.conv1
    conv_quan = model_quan.conv1
    load_ofwa(conv, conv_quan, None, 8, prefix=args.prefix+'/conv1')
    for layer_idx in range(1, 5):
        current_layer_pretrained = eval('model_pretrained.layer{}'.format(layer_idx))
        current_layer_quan = eval('model_quan.layer{}'.format(layer_idx))
        for block_idx in range(len(current_layer_pretrained)):
            ...
            conv_quan = current_block_quan.conv1
            q_module = current_block_quan.quan1
            load_ofwa(conv, conv_quan, q_module, args.weight_bit_width, prefix=args.prefix+'/layer'+str(layer_idx)+'_block'+str(block_idx)+'_conv1')

            # conv2
            conv = current_block_pretrained.conv2
            conv_quan = current_block_quan.conv2
            q_module = current_block_quan.quan2
            load_ofwa(conv, conv_quan, q_module, args.weight_bit_width, prefix=args.prefix+'/layer'+str(layer_idx)+'_block'+str(block_idx)+'_conv2')

            # downsample
            if current_block_pretrained.downsample is not None:
                conv = current_block_pretrained.downsample[0]
                conv_quan = current_block_quan.downsample[0]
                load_ofwa(conv, conv_quan, None, args.weight_bit_width, prefix=args.prefix+'/layer'+str(layer_idx)+'_block'+str(block_idx)+'_downsample')
    conv = model_pretrained.fc
    conv_quan = model_quan.fc
    q_module = model_quan.quan
    load_ofwa(conv, conv_quan, q_module, 8, prefix=args.prefix+'/fc')
    ...
    if args.scales:
        ...
        quantize(train_dataset, model_quan, args)
    ...
    save_state_dict(model_quan.state_dict(), args.prefix, filename='state_dict.pth')
...
def conduct_ofwa_act(train_loader, model_pretrained, model_quan, conv, conv_quan, q_module, act_bitwidth, prefix=None, ec=False):
    if q_module is None:
        return
    beta = act_quan_one_layer(train_loader, model_quan, q_module)
...
def load_ofwa_act(conv, conv_quan, q_module, act_bitwidth, prefix=None):
    if q_module is not None:
...
def conduct_ofwa(train_loader, model_pretrained, model_quan, conv, conv_quan, q_module, bitwidth, prefix=None, ec=False):
    # for fc
